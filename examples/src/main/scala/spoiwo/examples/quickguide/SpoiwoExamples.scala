...
import spoiwo.model.enums.{CellVerticalAlignment => VA, CellHorizontalAlignment => HA, CellReadingOrder => RO}
...
class SpoiwoExamples {
  ...
  def newSheetSavedWithOutputStream() =
    Workbook(
      Sheet(name = "new sheet"),
      Sheet(name = "second sheet"),
      Sheet(name = WorkbookUtil.createSafeSheetName("[O'Brien's sales*?]"))
  ...
  def creatingCells() =
    Sheet(name = "new sheet", row = Row().withCellValues(1, 1.2, "This is a string", true)).saveAsXlsx("workbook.xlsx")
  ...
  def creatingDateCells(): Unit = {
    ...
    Sheet(name = "new sheet", row = Row(simplestDateCell, formattedDateCell, formattedCalendarCell))
      .saveAsXlsx("workbook.xlsx")
  }
  ...
  def variousReadingOrder(): Unit = {
    ...
    val texts = List(
      (ro: CellReadingOrder) => List(persian, ro.toString, persian) mkString " ",
      (ro: CellReadingOrder) => List(ro.toString, persian, ro.toString) mkString " "
    )
    ...
    val cells = for {
      ...
      text(readingOrder),
      style = CellStyle(readingOrder = readingOrder, horizontalAlignment = HA.Center, verticalAlignment = VA.Center)
    )
  ...
  def createCell(ha: CellHorizontalAlignment, va: CellVerticalAlignment) =
  ...
  def variousAlignmentOptions(): Unit = {
    ...
    val alignments = List(
      HA.Center -> VA.Bottom,
      HA.CenterSelection -> VA.Bottom,
      HA.Fill -> VA.Center,
      HA.General -> VA.Center,
      HA.Justify -> VA.Justify,
      HA.Left -> VA.Top,
      HA.Right -> VA.Top
    )
  ...
  def workingWithBorders(): Unit = {
    val borders = CellBorders(
      ...
      topStyle = CellBorderStyle.MediumDashed,
      topColor = Color.Black
    )

    Sheet(name = "new sheet", row = Row(index = 1, Cell(value = 4, index = 1, style = CellStyle(borders = borders))))
  ...
  def workingWithFonts(): Unit = {
    ...
    Sheet(name = "new sheet", row = Row(index = 1, Cell("This is a test of fonts", 1, style)))
      .saveAsXlsx("workbook.xls")
  }

  def workingWithMultipleFonts(): Unit = {
    val cell = Cell(value = 0, style = CellStyle(font = Font(bold = true)))
    Sheet(name = "new sheet", rows = (0 to 10000).map(_ => Row(cell)).toList).saveAsXlsx("workbook.xls")
  }
  ...
  def customColors() =
    Sheet(
      Row(
        Cell(
          "custom XSSF colors",
          style = CellStyle(fillForegroundColor = Color(128, 0, 128), fillPattern = CellFill.Solid)
        )
      )
    )
  ...
  def setPrintArea() =
    Sheet(name = "Sheet1", properties = SheetProperties(printArea = CellRange(0 -> 0, 0 -> 1)))
      .saveAsXlsx("workbook.xlsx")

  def setPageNumbersOnFooter() =
    Sheet(name = "format sheet", footer = Footer.Standard(right = "Page &P of &N")).saveAsXlsx("workbook.xlsx")
  ...
  def insertImage() =
    Sheet(name = "Sheet1", images = List(Image(CellRange(1 -> 1, 1 -> 1), "examples/src/main/resources/einstein.jpg")))
      .saveAsXlsx("workbook.xlsx")
